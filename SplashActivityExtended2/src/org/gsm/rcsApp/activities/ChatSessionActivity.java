...
public class ChatSessionActivity extends Activity implements Runnable {
	...
	static String DESTINATION_ADDRESS;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Contact retrievedContact = intent
				.getParcelableExtra(MainActivity.SELECTED_CONTACT);

		displayName = retrievedContact.getDisplayName();
		if (displayName == null)
			displayName = retrievedContact.getContactId();
			...
			public void handleMessage(Message msg) {
				...
				if (msg.getData() != null) {
					...
					if (data.containsKey(RECEIVED_MESSAGE)) {
						ChatMessage receivedMessage = (ChatMessage) data
								.get(RECEIVED_MESSAGE);
						if (receivedMessage != null) {
							contactState.getMessageBuffer()
									.add(receivedMessage);
						}
					}
				}
				messageListView.smoothScrollToPosition(contactState
						.getMessageBuffer() != null ? (contactState
						.getMessageBuffer().size() - 1) : 0);
	...
	public void sendFileClicked(View view) {
		Intent intent = new Intent(_instance, FileTransferActivity.class);
		intent.putExtra(DESTINATION_ADDRESS, destinationUri);
	 ...
	 */
	public static boolean refreshMessageList(String contactUri,
			String recipient, ChatMessage chatMessage) {
	...
	private void sendAdhocMessage(String message, ChatMessage chatMessage) throws UnsupportedEncodingException {
		try {
			...
			chatMessageJSON.put("reportRequest", "Sent"); // possible status
															// values are
															// "Sent,Delivered,Displayed,Failed"
				...
				client.post(_instance.getApplication().getApplicationContext(),
						...
						new RCSJsonHttpResponseHandler() {
							...
							public void onSuccess(JSONObject response,
									int errorCode) {
								Log.d("ChatSessionActivity",
										"sendAdhocMessage::success = "
												+ response.toString()
												+ " errorCode=" + errorCode);

								JSONObject resourceReference = Utils
										.getJSONObject(response,
												"resourceReference");
								String resourceURL = Utils
										.getJSONStringElement(
												resourceReference,
												"resourceURL");
								String messageId = Utils
										.getMessageIdFromResourceURL(
												resourceURL, destinationUri);
								Log.d("ChatSessionActivity", "messageId="
										+ messageId + " resourceURL="
										+ resourceURL);

								if (errorCode == 201) {
									ContactStateManager
											.setMessageIdForSentMessage(
													messageInternalId,
													messageId, resourceURL);
									ContactStateManager.updateStatusFor(
											messageId,
											ChatMessage.MESSAGE_STATUS_SENT);
									messageHandler.sendEmptyMessage(0);
								}
							}
						});
			...
			Log.e("ChatSessionActivity", "Authentication Error: send adhoc message");
		}
	...
	public void run() {
		...
		while (running) {
			try {
				...
				if (ContactStateManager.haveSentMessageTo(destinationUri)
						&& running) {
					EditText messageInputBox = (EditText) findViewById(R.id.message_input_box);
					...
					if (trimmed.length() > 0) {
						...
						if ((now - lastSent) >= (COMPOSINGUPDATEFREQUENCYSECONDS * 1000)) {
							sendComposingIndicator(SplashActivity.userId,
									destinationUri, "active",
									COMPOSINGUPDATEFREQUENCYSECONDS,
									new java.util.Date(), "text/plain");
							lastSent = now;
						}
						sentComposing = true;
	 ...
	 */
	private void sendComposingIndicator(String userId, String contactId,
			String state, int refresh, java.util.Date lastActive,
			String contentType) {
		final String pingUrl = ServiceURL
				.getSendIsComposingAutomaticContactURL(SplashActivity.userId,
						contactId);
		...
		try {
			...
			try {
				...
				client.post(_instance.getApplication().getApplicationContext(),
						pingUrl, requestData, "application/json",
						new RCSJsonHttpResponseHandler() {
							@Override
							public void onSuccess(JSONObject response,
									int statusCode) {
								Log.d("ChatSessionActivity",
										"sendComposingIndicator::response = "
			...
			Log.e("ChatSessionActivity", "Authentication Error: send composing indicator");
		}
	}
	...
	/*
	 * called from the MainActivity when a message (send) status has been
	 * updated
	 */
	...
	/*
	 * called from the MainActivity when the isComposing indicator has been
	 * received
	 */
